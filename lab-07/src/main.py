...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.tab3 = self.ui.tabWidget.widget(2)
        self.tab4 = self.ui.tabWidget.widget(3)

        self.EIQty: QSpinBox = self.tab1.findChild(QLineEdit, 'EIEdit')
        self.EOQty: QSpinBox = self.tab1.findChild(QLineEdit, 'EOEdit')
        self.EQQty: QSpinBox = self.tab1.findChild(QLineEdit, 'EQEdit')
        self.ILFQty: QSpinBox = self.tab1.findChild(QLineEdit, 'ILFEdit')
        self.EIFQty: QSpinBox = self.tab1.findChild(QLineEdit, 'EIFEdit')
    ...
    def get_fp_qty(self):
        return {
            'EI': int(self.EIEdit.value()),
            'EO': int(self.EOEdit.value()),
            'EQ': int(self.EQEdit.value()),
            'ILF': int(self.ILFEdit.value()),
            'EIF': int(self.EIFEdit.value()),
        }
    ...
    def calculate_comp(self):
        ...
        time = round(3 * math.pow(labor, 0.33 + 0.2 * (1.01 - 1.01)), 2)
        budget = round(avg_salary * labor, 2)
